module Api
  module V1
    class BookingDetailsController < ApplicationController
      ...
      def index
        boking_details = BookingDetail.joins("INNER JOIN unit_details b on booking_details.unit_id=b.id INNER JOIN users c on booking_details.booked_by_user_id=c.id").select('booking_details.id as booking_id ,*').where('is_booked=true and booking_details.is_active=true')
        render json: {status: '1', msg: 'All booking details Loaded', data: boking_details}, status: :ok
      end

      def show
        booking_details = BookingDetail.joins("INNER JOIN unit_details b on booking_details.unit_id=b.id INNER JOIN users c on booking_details.booked_by_user_id=c.id").select('booking_details.id as booking_id ,*').where('booking_details.id=?',params[:id])
            ...
            File.open(path, "wb") do |f|
              ...
              if File.exist?(path)
                ...
                if booking_detail.save
                  ...
                  log = Log.new(:unit_number=>unit_details[:unit_number], :user_id=>booking_detail[:booked_by_user_id], :action=>action, :remark=>"New Booking", :admin_user_id=>1)
                  log.save
                  ## send notification
