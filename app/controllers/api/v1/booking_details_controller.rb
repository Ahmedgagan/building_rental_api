module Api
  module V1
    class BookingDetailsController < ApplicationController
      ...
      def index
        boking_details = BookingDetail.joins("INNER JOIN unit_details b on booking_details.unit_id=b.id INNER JOIN users c on booking_details.booked_by_user_id=c.id").select('booking_details.id as booking_id, c.name as agent_name , c.contact as agent_contact,*').where('is_booked=true and booking_details.is_active=true')
